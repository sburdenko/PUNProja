...
public class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
    ...
    private void LeftRoomCleanup()
    {
        ...
        if (autoCleanupSettingOfRoom)
        {
            ...
            List<GameObject> goList = new List<GameObject>( instantiatedObjects.Values );
        ...
        if(wasInRoom) SendMonoMessage(PhotonNetworkingMessage.OnLeftRoom);
    }
    ...
    void RebuildPlayerListCopies()
    {
        ...
        foreach (PhotonPlayer oPlayer in mActors.Values)
            if (!oPlayer.isLocal)
                otherP.Add(oPlayer);
        mOtherPlayerListCopy = otherP.ToArray();
    }
    ...
    public bool OpCreateGame(string gameID, bool isVisible, bool isOpen, byte maxPlayers, bool autoCleanUp, Hashtable customGameProperties, string[] propsListedInLobby)
    {
        ...
        return base.OpCreateGame(gameID, isVisible, isOpen, maxPlayers, autoCleanUp, customGameProperties, this.GetLocalActorProperties(), propsListedInLobby);
    }
    ...
    public void OnEvent(EventData photonEvent)
    {
        ...
        switch (photonEvent.Code)
        {
            ...
            case PhotonNetworkMessages.SendSerialize:
                ...
                this.ReadOnSerialize(serializeData, originatingPlayer);
                break;
    ...
    public void RemoveInstantiatedGO(GameObject go, bool localOnly)
    {
        ...
        for (int j = views.Length - 1; j >= 0; j--)
        {
            ...
            if (!removedFromServer)
            {
                ...
                if(view.owner != null) removeForActorID = view.owner.ID;
    ...
    private void DeltaCompressionWrite(PhotonView view, Hashtable data)
    {
        ...
        {   //We can compress as we have send a full update previously
            ...
            foreach (object newObj in uncompressedContents)
            {
                ...
                if (ObjectIsSameWithInprecision(newObj, oldObj))
                {
    ...
    private Dictionary<Type, Dictionary<PhotonNetworkingMessage, MethodInfo>> cachedMethods =
        new Dictionary<Type, Dictionary<PhotonNetworkingMessage, MethodInfo>>();
